...
MRL、LLM-to-SLM、Latent-Guided Reasoning、C2C 与 OverFill 已分别提供嵌套表示、跨模型 latent 生成、latent 推理指导、共享多接收器以及“大模型 Prefill—小模型 Decode”的存在性证据；首轮单学生满宽实验用于实现对齐和正对照，不作为重新证明这些组件可行或主张其新颖性的研究贡献。`[verified: MRL §3；LLM-to-SLM §3.1–3.2；Latent-Guided Reasoning §3.2–3.4；C2C 附录 A.5.2及表14；OverFill Abstract]`
...
| 阶段 | 最小实验 | 进入下一阶段的条件 | 未通过时的处理 |
|---|---|---|---|
| 实现对齐 | 一个教师、一个学生、一个满宽接口；先在少量样本上过拟合，再检查独立小验证集。`[opinion]` | 满宽 latent 能学习且优于匹配的无 latent、常量和错配控制。`[opinion]` | 检查梯度、注入位置、attention mask、标签对齐、汇聚器与读取器容量；不把实现失败写成方法结论。`[opinion]` |
| 单学生有序前缀 | 固定 slot 数，比较多个嵌套宽度的 multi-width、full-only truncate 与少量独立宽度接口。`[opinion]` | 非满宽前缀产生稳定收益，multi-width 在短前缀优于 full-only truncate，且满宽质量无不可接受退化。`[opinion]` | 独立宽度有效而嵌套训练无效时，转向离散宽度接口；只有满宽有效时，先定位容量、汇聚或宽度采样问题。`[opinion]` |
| 跨学生共享排序 | 加入第二个同家族、不同规模学生，完整运行四格因子实验。`[opinion]` | shared 在每个学生的多数宽度落入预注册的 private 非劣区间，且没有稳定负迁移。`[opinion]` | private 有效而 shared 失败时，转向跨学生表示冲突、分组共享或轻量学生私有残差。`[opinion]` |
| 组合泛化 | 留出中间宽度；冻结教师侧协议后只训练新学生读取器与匹配的学生适配模块。`[opinion]` | 未见宽度保持相邻宽度趋势，新学生以显著更低的接入成本恢复大部分联合重训收益。`[opinion]` | 留出宽度失败则只主张离散预算菜单；新学生失败则收缩为已训练学生的多预算接口。`[opinion]` |
| 任务与成本扩展 | 数学先跨难度复现，再测试闭卷知识、教师独占证据和真实工具执行闭环；最后绘制逐学生 quality–width–cost 曲线。`[opinion]` | 至少一个任务族跨数据集稳定复现，并在明确目标质量区间进入质量—通信或质量—延迟 Pareto 前沿。`[opinion]` | 按下文结果分支收缩到推理指导、证据压缩、静态工具规划或功能性接口，不自动保留通用知识与效率主张。`[opinion]` |
...
| 观测结果 | 可支持的判断与下一步 |
|---|---|
| 输入相关 latent 胜过匹配的无 latent、常量与错配控制 | 当前实现具备可用的信息通道；继续检验有序前缀，不能把该结果单独作为 COPE 的新颖贡献。`[opinion]` |
| 常量或错配 latent 与正确 latent 相当 | 当前收益不能归因于样本相关教师信息；停止扩展实验矩阵，先检查普通微调收益、读取器捷径与控制组是否匹配。`[opinion]` |
| 高预算或长前缀有效，短前缀无效 | 优先定位宽度、slot 汇聚或前缀训练的瓶颈；该结果不否定知识可以经 latent 传递。`[opinion]` |
| 有序前缀与共享接口均成立，组合后也无明显负交互 | 支持完整 COPE 主线；继续验证留出宽度、新学生轻量接入以及质量—宽度—学生能力匹配规律。`[opinion]` |
| 学生专用 multi-width 有效，共享接口无效 | 直接挑战 COPE 的跨学生共享主张；转向跨学生表示冲突、student-conditioned normalization、分组共享或低秩学生私有残差。`[opinion]` |
| 共享 full-only 有效，多宽度监督无独立收益 | 只能支持普通共享 latent 接口；有序前缀贡献不成立，且共享 projector 与多接收器已有直接先例。`[opinion]` |
| 有序前缀与共享接口分别有效，但 shared multi-width 明显退化 | 表明共享约束与信息排序存在负交互；转向梯度冲突、坐标使用差异与部分共享机制研究。`[opinion]` |
| 未见宽度不能保持相邻宽度趋势 | 将部署表述为经过训练的离散预算菜单，不主张连续宽度泛化。`[opinion]` |
| 冻结协议后新学生接入失败 | 将 composable 范围收缩为训练阶段覆盖的学生；新学生需要联合重训或学生分组协议。`[opinion]` |
...
| 教师对、学生错的纠正率提高，但学生原本正确的样本也频繁被改错 | latent 有影响但不是稳定增益通道；转向 selective collaboration，预测何时读取、忽略或缩短 latent。`[opinion]` |
| 静态工具选择改善，但读取工具结果后的后续决策无提升 | 只支持一次 Prefill 下的静态工具规划；动态工具协同需要信息刷新或增量编码，不能由增加前缀宽度替代。`[opinion]` |
