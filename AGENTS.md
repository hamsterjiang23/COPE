...
> **当前状态入口（2026-09-14）：** 先读 [当前目标、代码与路线](docs/project-status-and-roadmap.md) 和 [实验索引](goal/README.md)。两端冻结、只训练接口；有效信息增益优先，学生跳过问题 Prefill 是候选。v1–v3 已完成，尚未确认输入相关收益。下文带日期的准备/启动/未实现条目保留为历史快照，以后续完成记录和各版结果为准。
...
训练从多学生、多宽度的生成损失开始：`L = Σ_s Σ_d λ[s,d] · NLL(S_s(y | x_s, A_s(Z[:, :d])))`。所有分支更新同一教师侧接口；学生采样、宽度采样与损失权重必须记录。**教师与学生均完全冻结，只训练中间套娃接口（可学习汇聚器、projector、reader）；不得添加学生 LoRA、微调学生或教师。** 教师提取问题输入的最后一层 hidden；学生前向保留输入梯度以更新接口，冻结权重不等于对学生前向使用 no_grad。`[verified: 用户 2026-09-13 纠正训练边界]`
...
| 组合泛化 | 留出中间宽度；冻结教师侧协议与新学生，只训练新学生读取器。`[opinion]` | 未见宽度保持相邻宽度趋势，新学生以显著更低的接入成本恢复大部分联合重训收益。`[opinion]` |
...
## 实验版本管理（2026-09-13 用户规定）

- 所有研究实验必须在 `goal/` 下按 `v1`、`v2`、`v3` 顺序迭代，不复用版本号。`[verified: 用户指定版本约定]`
- 每版记录目标、前版变化、固定配方、过程、结果和下一步判断；版本总索引见 `goal/README.md`。`[verified: 用户批准的实施计划]`
- 同一配方的重跑、随机种子及恢复使用独立 run ID；改变研究问题、模型组合、数据协议或主要方法时创建新版本。`[verified: 用户批准的实施计划]`
- 已执行 run 的配置及完成结果不得覆盖；更正以追加日志、新 run 或新版本记录。运行中事件及检查点可以追加，终态文件独占创建。`[verified: 用户批准的实施计划]`
- 小型配置、样本 ID、指标、manifest 和产物索引纳入 Git；完整日志、预测、原始数据和权重保存在服务器独立目录，不提交 Git。记录代码 commit、未提交补丁与哈希、模型和数据 revision、环境、随机种子、设备、预算和 pueue ID。`[verified: 用户批准的实施计划]`
- 公共实现继续位于 `src/cope/`，版本目录保存配方与证据，不复制框架；新增版本前必须阅读上一版结论及失败记录。`[verified: 用户批准的实施计划]`
- 当前首版为 `goal/v1`：Qwen3-14B → Qwen3-1.7B，满宽可行性和匹配训练对照，计算预算六小时。实际状态以该目录 journal、results 和 run 证据为准，不能将准备完成写为实验成功。`[verified: 用户批准的 v1 配方]`

### v1 实际状态（2026-09-13，更新此前 2026-09-08 实现快照）

已完成真实模型入口、LoRA、双 GPU 放置、GSM8K 固定划分及单学生满宽四组对照；30 项功能测试通过。run `v1-20260913T075729Z-r01` 四组均训练 128 步，七项评测均完成 128 题，耗时 54.68 分钟。正确 latent 准确率 68.75%，匹配无 latent 64.06%，差异的 95% 配对区间包含零，且正确 latent 未优于常量与错配控制；结论为不确定，不满足输入相关教师信号门控。原始学生得分受严格格式解析严重影响，不可直接解释为能力差距。后续读取 `goal/v1/results.md` 和完整 run 证据；不自动启动 v2。`[verified: goal/v1/runs/v1-20260913T075729Z-r01/metrics.json；goal/v1/results.md]`

### v2 纠正（2026-09-13）

用户明确要求教师与学生均冻结，仅训练中间套娃模块，并同意新建 `goal/v2`。v1 学生 LoRA 与仅满宽实验属于偏离目标的历史探索，不作为双冻结套娃方法的有效性验证。v2 暂定多前缀监督与 full-only truncate 对照；当前只完成配方准备和功能测试，尚未启动真实模型实验。后续以 `goal/v2` 记录为准。

### v2 启动（2026-09-13）

用户明确要求开始实验。run `v2-20260913T100946Z-r01` 已于 18:09 CST 启动，pueue 任务 16；两端冻结，多宽度训练仅更新中间模块。尚无留出集结果，状态以 `goal/v2/journal.md` 和 `results.md` 为准；六小时上限，不自动进入 v3。

### v2 完成（2026-09-13，更新以上启动状态）

run `v2-20260913T100946Z-r01` 于 20:38 CST 成功完成，四组均 128 步，19 项评测各 128 题，累计 148.81 分钟。教师和学生均冻结，仅中间接口训练。正确多宽度接口在 256/512 维优于 full-only 截断，但满宽退化，且未可靠优于常量和错配控制；输入相关性门控未通过，不自动扩展多学生或启动 v3。冻结基线受严格答案格式解析混杂；代码在复制满宽 latent 后才截取，逻辑前缀 payload 不等于实际传输量缩小。结果和完整性/通信审计见 `goal/v2/results.md` 及该 run 证据。`[verified: goal/v2/runs/v2-20260913T100946Z-r01/metrics.json；integrity-audit.json；transport-audit.json]`

### 用户再次纠正：学生直接 Decode（2026-09-13，优先于前述 embedding-prefix 配方）

用户要求：教师执行问题 Prefill，最后一层 hidden output 输入套娃接口；接口输出供冻结学生直接自回归 Decode 使用的状态。学生不再对原问题执行 embedding 或 Prefill。教师与学生继续完全冻结，仅训练中间接口。此前将接口输出作为连续 prefix embeddings 并拼接完整学生问题的 v2 实现偏离该计算路径，不能作为此目标方法的验证。`[verified: 用户本轮明确纠正；src/cope/adapters.py _prepare_inputs 与 generate]`

实现前必须区分学生 hidden-size 向量与完整 Decode 状态：标准 Qwen 持续利用上下文需各层 KV cache、位置状态及首步输入/首 token 启动约定。不能把 inputs_embeds 当作跳过 Prefill，不能把维度相同当作状态兼容。是否由套娃接口构造各层 cache，以及“不进行 embedding”是否也包含新生成 token，尚需明确，不能擅自认定用户已选择 KV 映射或连续 hidden 递归方案。当前仅记录更正，不改写已执行 run，不启动新实验。

### v3 目标更新（用户确认有效信息传递优先）

用户确认核心目标为大模型信息带来小模型增益，并授权 v3 多组合探索。教师与学生均冻结、禁止 LoRA；学生跳过问题 Prefill 调整为候选路线而非所有组合的硬约束。后续生成 token 正常 embedding，先前对此尚未明确的记录已由用户澄清。v3 以 3 种接口 × 2 个教师层位筛选，固定单宽度；不把本轮当作 MRL 多宽度或跨学生共享验证。开发选择与独立留出严格分离，允许无候选通过；六小时总预算，实际状态见 goal/v3，不自动进入 v4。

### v3 完成与仓库整理（2026-09-14）

run `v3-20260913T140917Z-r02`（pueue 18）完成六候选与两个附加控制的 128 步训练，以及 20 份校准/开发/留出评测。开发选择 middle_embedding；留出正确 55.86%、训练错配 59.38%、推理错配 56.25%，确认未通过。原始学生 197/256 题未解析，不将相对学生的全部得分提升归因于信息传递。累计 176.28 分钟，CPU 完整性审计及 42 项测试通过。原始证据不覆盖，当前不自动注册或启动 v4。下一阶段优先评测/信号/读取诊断，证据通过后再做多宽度、跨学生与跨数据集；详见 `goal/v3/results.md` 和 `docs/project-status-and-roadmap.md`。`[verified: v3 metrics and integrity-audit; CPU pytest run 2026-09-14]`
